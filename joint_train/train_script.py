...
from jointlosstrainer import JointLossTrainer
import yaml
import argparse
...
def main(configs):
    ...
    prm_compute_metrics = get_compute_metrics(do_gen=configs.do_gen if 'do_gen' in configs else True,
                                              model_type=configs.model_type if 'model_type' in configs else 'llama')
    ...
    # model.lm_head = torch.nn.Linear(in_features=in_features, out_features=2, bias=False) 
